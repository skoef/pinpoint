...
- **Waypoint images are stored under `waypoints/<waypoint id>/`** via the `waypoint_image_path` callable in `models.py`. This means a `Waypoint` must be saved *before* its image is attached, or the file lands under `waypoints/None/` — both write paths in `views.py` deliberately create the row first and attach the image in a second `save()`. Never build image URLs by prepending scheme/host: `image.url` is already absolute under S3 storage, and doing so yields `https://app/https://s3…`.
